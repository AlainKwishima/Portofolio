...
export function ProjectCard({ projects }: { projects: ProjectProps[] }) {
  ...
  return (
    <div className='flex flex-col gap-8'>
      {projects.map(({ title, description, tags, image, video, link, slug }, idx) => {
        // If we have a slug, we use translated content, otherwise fallback to provided data (for GitHub repos)
        const hasTranslation = slug && t.has(`items.${slug}`)
        const localizedTitle = hasTranslation ? t(`items.${slug}.title`) : title
        const localizedDescription = hasTranslation ? t(`items.${slug}.description`) : description
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      {link.preview ? (
                        ...
                        >
                          <span className="group-hover:text-primary dark:group-hover:text-primary transition-colors">{localizedTitle}</span>
